...
bool Core::init(kernel::DeviceManager& dm) {
    if (state == core_state::BOOTSTRAP) { init_dts(); }
    return true;
}
...
void Core::init_dts()
{
    ...
    syslog.messagef(LogLevel::DEBUG, "GDTR = %p, IDTR = %p, TR = %p", gdt_.get_ptr(), idt.get_ptr(), tss_.get_sel());
}
...
bool Core::timer_init() {
    ...
    // Set the timer to be periodic, with a period of 4ms, and start the timer.
